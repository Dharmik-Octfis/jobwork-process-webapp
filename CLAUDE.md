...
| `docs/SSO_LOGIN_API_SEQUENCE.md`                          | …and the same sign-in as a reference card: every API call in order with request and response, plus the returning, silent and direct-entry variants                                                                                                                                                                                                                                                                                                          |
...
| `docs/SSO_WEBSITE_ENTRY_PLAN.md`                          | ✅ **deployed 2026-09-21 except phase B** (the two jobwork URL values, waiting for the page to be public). Sign-in moves from jobwork's `/login` to a public product site (`https://www.octfis.com/job-work-1` — exact string, `www` included); what changes on each of the three sites, the traps, the build order. Read §2 before touching the `_session` cookie and §4.5 before adding a post-logout URI                                                 |
