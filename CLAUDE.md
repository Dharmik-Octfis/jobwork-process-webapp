...
| `docs/SSO_WEBSITE_ENTRY_PLAN.md`                          | ❌ **plan, nothing built.** Sign-in moves from jobwork's `/login` to a public product site (`octfis.com/jobwork`); what changes on each of the three sites, the traps, the build order. Read §2 before touching the `_session` cookie and §4.5 before adding a post-logout URI                                                                                                                                                                              |
