...
| `docs/SSO_AND_IDENTITY.md`                                | …and the design behind it. **built, not deployed** — `accounts/` runs locally, and `SSO_ENABLED=true` in `backend/.env`, so local dev signs in through it. Staging and production leave the flag unset, so password login is still the only way in there. Its header table says what is built |
