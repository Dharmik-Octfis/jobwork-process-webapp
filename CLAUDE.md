...
| `docs/SSO_AND_IDENTITY.md`                                | **built, not deployed** — one login across every app. `accounts/` runs locally; `SSO_ENABLED` is off everywhere, so password login is still the only way in. Its header table says what is built |
