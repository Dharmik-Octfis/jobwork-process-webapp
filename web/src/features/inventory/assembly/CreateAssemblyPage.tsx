...
export function CreateAssemblyPage() {
  ...
  const updatePreferenceMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      form.setValue('assemblyNumber', `${data.prefix}${data.nextNumber.toString().padStart(5, '0')}`, { shouldValidate: true });
  ...
  const handleServiceChange = <K extends keyof { itemId: string; qtyRequired: number; costPrice?: number }>(
    index: number,
    field: K,
    value: { itemId: string; qtyRequired: number; costPrice?: number }[K]
  ) => {
  ...
  const createMutation = useMutation({
    ...
    onError: (error: unknown) => {
      ...
      toast.error(err.response?.data?.message || (error instanceof Error ? error.message : 'Failed to create assembly'));
    },
  });
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: '#fff' }}>
      {/* Header */}
      <div style={{ padding: '16px 24px', borderBottom: '1px solid #eef0f3', display: 'flex', alignItems: 'center', gap: '8px' }}>
        <PackageIcon />
        <h1 style={{ margin: 0, fontSize: '20px', fontWeight: 600, color: '#1e293b' }}>New Assembly</h1>
      </div>

      <div style={{ flex: 1, overflowY: 'auto', padding: '24px' }}>
        <form id="create-assembly-form" onSubmit={handleSubmit(onSubmit, onValidationError)} style={{ maxWidth: '900px' }}>
          {/* Header Form */}
          <div style={{ display: 'grid', gridTemplateColumns: '200px 1fr', gap: '24px', alignItems: 'start', marginBottom: '40px' }}>

            <div style={{ fontSize: '13px', color: '#dc2626', fontWeight: 500, paddingTop: '8px' }}>Composite Item*</div>
              ...
              <Controller
                ...
                render={({ field }) => (
                  <SearchableSelect
                    ...
                    options={(compositeItems || []).map(item => ({ value: item.id, label: item.name }))}
                    placeholder="Select or type to add"
                    hasError={!!errors.compositeItemId}
                  />
                )}
              />
              {errors.compositeItemId && <div style={{ color: '#dc2626', fontSize: '12px', marginTop: '4px' }}>{errors.compositeItemId.message}</div>}
              {compositeItemId && (
          ...
          >
            ...
            <div style={{ maxWidth: '400px' }}>
              <div style={{ display: 'flex', alignItems: 'center', border: '1px solid #d1d5db', borderRadius: '4px', overflow: 'hidden' }}>
                <input
                  ...
                  style={{ flex: 1, border: 'none', padding: '8px 12px', fontSize: '13px', outline: 'none' }}
                  placeholder="Auto Generated"
                />
                <button type="button" onClick={() => setIsNumberConfigOpen(true)} style={{ background: '#f8fafc', border: 'none', borderLeft: '1px solid #d1d5db', padding: '8px 12px', cursor: 'pointer' }}>
                  <Settings size={14} color="#0062ff" />
                </button>
              </div>
              {errors.assemblyNumber && <div style={{ color: '#dc2626', fontSize: '12px', marginTop: '4px' }}>{errors.assemblyNumber.message}</div>}
            </div>

            <div style={{ fontSize: '13px', color: '#1e293b', paddingTop: '8px' }}>Description</div>
            <div style={{ maxWidth: '400px' }}>
              <textarea
            ...
            <div style={{ fontSize: '13px', color: '#dc2626', fontWeight: 500, paddingTop: '8px' }}>Assembled Date*</div>
            <div style={{ maxWidth: '400px' }}>
              <Input type="date" label="" error={errors.assemblyDate?.message} {...form.register('assemblyDate')} />
            </div>

            <div style={{ fontSize: '13px', color: '#dc2626', fontWeight: 500, paddingTop: '8px' }}>Quantity to Assemble*</div>
            <div style={{ maxWidth: '400px' }}>
              <Input type="number" min="1" step="any" label="" error={errors.qty?.message} {...form.register('qty', { valueAsNumber: true })} />
              <div style={{ fontSize: '12px', color: '#64748b', marginTop: '4px' }}>
                You can Assemble <strong style={{ color: '#1e293b' }}>0</strong> unit from the available stock.
              </div>
            </div>

            <div style={{ fontSize: '13px', color: '#dc2626', fontWeight: 500, paddingTop: '8px' }}>Location*</div>
            <div style={{ maxWidth: '400px' }}>
              <Controller
                name="locationId"
                control={control}
              ...
              />
              {errors.locationId && <div style={{ color: '#dc2626', fontSize: '12px', marginTop: '4px' }}>{errors.locationId.message}</div>}
            </div>
          ...
          <div style={{ marginBottom: '16px' }}>
            <div style={{ fontSize: '13px', color: '#dc2626', fontWeight: 500, marginBottom: '16px' }}>Associated Items*</div>

            <div style={{ background: '#eff6ff', padding: '12px 16px', borderRadius: '4px', display: 'flex', alignItems: 'flex-start', gap: '8px', marginBottom: '24px', fontSize: '13px', color: '#1e40af' }}>
              <Info size={16} style={{ marginTop: '2px', flexShrink: 0 }} />
              <div>If you've incurred an addition cost while assembling this item such as rent, labour, or scrap; you can <strong>add it as a service item</strong> to associate that cost to the item.</div>
            </div>

            <table style={{ width: '100%', borderCollapse: 'collapse', border: '1px solid #d1d5db', fontSize: '13px' }}>
              <thead>
                <tr style={{ background: '#f8fafc', borderBottom: '1px solid #d1d5db' }}>
                  <th style={{ padding: '12px 16px', textAlign: 'left', fontWeight: 500, color: '#475569', borderRight: '1px solid #d1d5db' }}>Item Details</th>
                  <th style={{ padding: '12px 16px', textAlign: 'right', fontWeight: 500, color: '#475569', borderRight: '1px solid #d1d5db', width: '160px' }}>Quantity Required</th>
                  <th style={{ padding: '12px 16px', textAlign: 'right', fontWeight: 500, color: '#475569', borderRight: '1px solid #d1d5db', width: '140px' }}>Total Qty required</th>
                  <th style={{ padding: '12px 16px', textAlign: 'right', fontWeight: 500, color: '#475569', width: '140px' }}>Quantity Available</th>
                </tr>
              </thead>
              <tbody>
                {componentsLoading ? (
                  <tr>
                    <td colSpan={4} style={{ padding: '24px', textAlign: 'center', color: '#64748b' }}>Loading components...</td>
                  </tr>
                ) : !goodsComponents || goodsComponents.length === 0 ? (
                  <tr>
                    <td colSpan={4} style={{ padding: '24px', textAlign: 'center', color: '#64748b' }}>Select a composite item to view components.</td>
                  </tr>
                ) : (
                        ...
                        <tr style={{ borderBottom: '1px solid #eef0f3' }}>
                          <td style={{ padding: '16px', borderRight: '1px solid #eef0f3' }}>
                            <div style={{ display: 'flex', gap: '12px', alignItems: 'flex-start' }}>
                              <div style={{ width: '40px', height: '40px', background: '#f1f5f9', display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: '4px', border: '1px solid #eef0f3' }}>
                                <ImageIcon size={20} color="#94a3b8" />
                              </div>
                              <div>
                                <div style={{ fontWeight: 500, color: '#1e293b' }}>{comp.component?.name}</div>
                                <div style={{ fontSize: '11px', color: '#64748b', marginTop: '2px' }}>SKU: {comp.component?.sku || '-'}</div>
                              </div>
                            </div>
                            <div style={{ marginTop: '16px' }}>
                              <input type="text" placeholder="Add a description to your item" style={{ width: '100%', border: 'none', background: 'transparent', outline: 'none', fontSize: '13px', color: '#475569' }} />
                            </div>
                          </td>
                          <td style={{ padding: '16px', textAlign: 'right', borderRight: '1px solid #eef0f3', verticalAlign: 'top' }}>
                            <div style={{ fontWeight: 500, color: '#1e293b' }}>{requiredPerUnit}</div>
                            <div style={{ fontSize: '11px', color: '#64748b', marginTop: '4px' }}>x 1 assemblies</div>
                          </td>
          ...
          {(services.length > 0 || serviceComponents.length > 0) && (
            ...
            >
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', border: '1px solid #d1d5db', fontSize: '13px', marginTop: '8px' }}>
                <thead>
                  <tr style={{ background: '#f8fafc', borderBottom: '1px solid #d1d5db' }}>
                    <th style={{ padding: '12px 16px', textAlign: 'left', fontWeight: 500, color: '#475569', borderRight: '1px solid #d1d5db' }}>Service Details</th>
                    <th style={{ padding: '12px 16px', textAlign: 'right', fontWeight: 500, color: '#475569', borderRight: '1px solid #d1d5db', width: '160px' }}>Quantity Required</th>
                    <th style={{ padding: '12px 16px', textAlign: 'right', fontWeight: 500, color: '#475569', borderRight: '1px solid #d1d5db', width: '140px' }}>Total Qty required</th>
                    <th style={{ padding: '12px 16px', textAlign: 'right', fontWeight: 500, color: '#475569', borderRight: '1px solid #d1d5db', width: '140px' }}>Cost per unit</th>
                    <th style={{ padding: '12px 16px', textAlign: 'center', fontWeight: 500, color: '#475569', width: '40px' }}></th>
                  </tr>
                </thead>
                <tbody>
                      ...
                      <React.Fragment key={comp.id}>
                        <tr style={{ borderBottom: '1px solid #eef0f3' }}>
                          <td style={{ padding: '16px', borderRight: '1px solid #eef0f3' }}>
                            ...
                            <div style={{ marginTop: '16px' }}>
                              <input type="text" placeholder="Add a description to your service" style={{ width: '100%', border: 'none', background: 'transparent', outline: 'none', fontSize: '13px', color: '#475569' }} />
                            </div>
                          </td>
                          <td style={{ padding: '16px', textAlign: 'right', borderRight: '1px solid #eef0f3', verticalAlign: 'top' }}>
                            <div style={{ fontWeight: 500, color: '#1e293b' }}>{requiredPerUnit}</div>
                          ...
                          <td style={{ padding: '16px', textAlign: 'right', borderRight: '1px solid #eef0f3', verticalAlign: 'top' }}>
                            <div style={{ fontWeight: 500, color: '#1e293b' }}>{comp.component?.costPrice || 0}</div>
                            <div style={{ fontSize: '11px', color: '#64748b', marginTop: '4px' }}>Total Cost:<br/>₹ {((comp.component?.costPrice || 0) * totalRequired).toLocaleString('en-IN')}</div>
                          </td>
                          <td style={{ padding: '16px', textAlign: 'center', verticalAlign: 'top', color: '#64748b' }}>
                            <span style={{ fontSize: 11 }}>(From Recipe)</span>
                          </td>
                    ...
                    return (
                      ...
                      <td colSpan={2} style={{ padding: '16px', borderRight: '1px solid #eef0f3' }}>
                        <input
                          ...
                          onChange={(e) => handleServiceChange(idx, 'qtyRequired', parseFloat(e.target.value) || 0)}
                          style={{
                            width: '100%',
                            padding: '6px',
                            border: '1px solid #cbd5e1',
          ...
          <div
            style={{
              padding: '16px',
              border: '1px solid #e2e8f0',
              borderTop: (services.length > 0 || serviceComponents.length > 0) ? 'none' : '1px solid #e2e8f0',
              borderRadius: (services.length > 0 || serviceComponents.length > 0) ? '0 0 8px 8px' : '8px',
              background: '#f8fafc',
      ...
      {/* Footer Actions */}
      <div style={{ padding: '16px 24px', borderTop: '1px solid #eef0f3', display: 'flex', gap: '12px', background: '#f9f9fb' }}>
        <button
          type="submit"
