...
/**
 * The stepper — the page people actually work from.
 *
...
export function JobOrderStepper({
  steps,
  onIssue,
  onReceive,
  onViewIssues,
  ...
  const unitFor = (step: OverviewStep, side: 'issue' | 'receive') => {
    const uom = side === 'issue' ? step.issueUom : step.receiveUom;
    return uom ? (uom.symbol ?? uom.unitName) : '';
  };
  ...
  return (
    <div>
      {steps.map((step) => {
         ...
         *
         *   - the step has closed. One still out at the dyer has issued
         *     everything and received nothing, which reads as 100% and says
         *     nothing at all.
         *   - 🔴 the units match. `issued − received` across a step that turns
         *     metres into pieces is not a quantity, and printing a number there
         *     would invite somebody to act on it.
         */
        const settled = step.status === 'completed' || step.status === 'short_closed';
        /**
         * 🔴 One item in, one item out, same unit — otherwise there is no
         * subtraction to do. "Issued − received" across three inputs and two
         * outputs is not a quantity at all, and a number printed here would be
         * acted on.
         */
        const comparable =
        ...
        return (
          <div key={step.id} style={cardStyle}>
              ...
              <div style={{ display: 'flex', gap: 12, alignItems: 'flex-start' }}>
                ...
                <div>
                  <h3 style={{ fontSize: 14, fontWeight: 600, color: '#111', margin: 0 }}>
                    {/* The SNAPSHOT, not a join — a process renamed since must not
                        retitle a step whose challans are already printed. */}
            ...
            {reworkPending && (
              ...
              >
                ...
                <span style={{ fontSize: 12, color: '#92400e', lineHeight: 1.5 }}>
                  {formatQty(step.totals.reworkQty)} {unitFor(step, 'receive')} came back needing
                  rework. It sits in its own lot — issue it back to this step to run it again.
                </span>
              </div>
            )}
            ...
            {/*
              ...
              A step consumes a set and returns a set, and the two are unrelated
              in length and in unit — three items in and two out is normal. A
              single "Issued / Received" pair could only ever describe one of
              them, so it described the principal one and quietly hid the rest.
            */}
            <div
              style={{
                display: 'grid',
            ...
            >
              {/*
                ...
                The plan is not repeated here. It is on the step when you edit
                the order, and — where it actually matters — in the Issue dialog,
                which shows planned, already issued, remaining and the tolerance
                ceiling at the moment somebody decides how much to send. Carrying
                it here as well is the same number in two places, free to be read
                as a target the step is failing.
              */}
              <FlowTable
                title="Consumed"
                columns={['Issued']}
            ...
            >
              ...
              <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
                {!step.canIssue && (
                  <span style={{ fontSize: 11, color: '#94a3b8', maxWidth: 340 }}>
                    {/* 🔴 The REASON, not just a disabled button. A greyed-out
                        control with no explanation is how somebody concludes the
                        software is broken. */}
