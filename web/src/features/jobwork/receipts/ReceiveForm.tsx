...
export function ReceiveForm({ jobOrder, step, onReceived, onCancel }: Props) {
  ...
  const { data: stockLocations = [] } = useQuery({
    ...
    enabled: Boolean(orgId) && stepItemIds.length > 0,
  });
    ...
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
      <div style={{ marginBottom: 24 }}>
        <h2 style={{ fontSize: 18, fontWeight: 600, color: '#111', margin: '0 0 4px 0' }}>
          Receive goods — step {step.seq}, {step.processNameSnapshot}
        </h2>
...
function toBatchPayload(batch: BatchAllocation): JobReceiptBatchAllocationData {
   ...
   * neither an existing one. The asymmetry is the server's rule, not an
   * oversight: an existing batch is never RESTAMPED from inside a receipt, but
   * three more rolls arriving into it is new goods, not a correction.
   *
