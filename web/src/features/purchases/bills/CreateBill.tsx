...
export function CreateBill() {
  ...
  const { data: locations = [] } = useQuery({
    ...
    // 🔴 Ours only. Goods on a bill are delivered to a place WE hold; a
    // jobworker's location exists so the ledger can say where our stock is
    // standing, and it is not an answer to "where should this arrive?".
    // Filtered on the query so the picker, the default and the address modal
    // below all inherit it rather than each remembering.
          ...
          onClick={() =>
            navigate(
              (location.state as { returnUrl?: string })?.returnUrl ||
                `/organizations/${orgId}/purchases/bills`,
            )
          }
          style={{
            background: 'none',
