...
interface VendorNumberConfigModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export function VendorNumberConfigModal({
  isOpen,
  onClose,
  onSave,
  initialPrefix = 'VEN-',
  initialNextNumber = '00727',
}: VendorNumberConfigModalProps) {
      ...
      <div style={{ backgroundColor: 'white', borderRadius: '0 0 8px 8px', width: '600px', maxWidth: '90%', boxShadow: '0 10px 25px rgba(0,0,0,0.1)' }}>
        ...
        <div style={{ padding: '24px' }}>
          <p style={{ margin: '0 0 24px 0', color: '#444', fontSize: '14px', lineHeight: '1.5' }}>
            Vendor numbers will be auto-generated based on the preferences below. For each new vendor that is created, the number after the prefix will be incremented by 1.
          </p>
          ...
          <div style={{ backgroundColor: '#fff9e6', padding: '16px', borderRadius: '6px', color: '#5c4813', fontSize: '13px', lineHeight: '1.5' }}>
            Note: If you want to change only this vendor's number without affecting the current series, you can edit it directly from the Vendor Number field after closing this popup.
          </div>
        </div>
