        ...
        lines: await Promise.all(
          assembly.lines.map(async (line) => {
            ...
            return {
              ...
              unitValue:
                Number(resolved.unitValue) > 0
                  ? Number(resolved.unitValue)
                  : Number(line.item.costPrice || 0),
              value:
                Number(resolved.value) > 0
                  ? Number(resolved.value)
                  : Number(line.qty) * Number(line.item.costPrice || 0),
            };
          }),
        ),
      };
