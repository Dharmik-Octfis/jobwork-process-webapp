...
describe('vendors — cross-tenant isolation', () => {
  ...
  it('a member CAN still read their own organisation’s vendors', async (ctx) => {
    ...
    const res = await request(createApp())
      .get(vendorsUrl(org.id))
      .set('Authorization', `Bearer ${token}`);

    expect(res.status).toBe(200);
    expect(Array.isArray(res.body)).toBe(true);
    // Exactly their own vendors — not "some", not "all of everyone's".
    expect(res.body.length).toBe(org.vendors);
    for (const vendor of res.body) expect(vendor.organizationId).toBe(org.id);
  });
