...
/**
 * Cross-tenant isolation for the customers module.
 *
 ...
 */
const customersUrl = (orgId: string) => `/api/organizations/${orgId}/sales/customers`;
describe('customers — cross-tenant isolation', () => {
  // The control. Without this, "everything returns 403" would look like a pass.
  it('a member CAN still read their own organisation’s customers', async (ctx) => {
    // `where: { customers: { some: {} } }` cannot be used to find this fixture:
    ...
    const res = await request(createApp())
      .get(`${customersUrl(org.id)}?perPage=100`)
    ...
    expect(results.length).toBe(org.customers);
  ...
  it('a malformed organization id is rejected, not 500', async () => {
    ...
    const res = await request(createApp())
      .get(customersUrl('not-a-uuid'))
      .set('Authorization', `Bearer ${token}`);

    expect(res.status).toBe(403);
  });

  it('a user cannot read the customers of an organization they do not belong to', async (ctx) => {
    // Pick the victim FIRST — an org that actually has customers to leak — then
  ...
  it('a user cannot create a customer inside an organization they do not belong to', async (ctx) => {
    ...
    // it would have landed. `prisma.customer.findFirst` on the global client is
    ...
    if (written) {
      await runAsTenant(victimOrg.id, (tx) =>
        tx.customer.deleteMany({ where: { id: written.id, organizationId: victimOrg.id } }),
      );
    }

    expect(
      written,
      `LEAK: user ${membership.userId} wrote a customer into "${victimOrg.name}", ` +
