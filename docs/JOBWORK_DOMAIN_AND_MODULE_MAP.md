...
⚠️ **The implementation was withdrawn on 2026-08-17. The problem above is real; `Process.requiresSingleBatch`
was the wrong place to solve it, and is now dropped from the column down.** Two things killed it:

1. It came off the Process form on 2026-08-12 alongside `preservesPackaging`, but the service kept
   spreading it into UPDATE as well as CREATE. With the form no longer sending the key, **every edit of a
   process silently reset the flag to false** — it could not be set, seen, or deliberately cleared.
2. The server guard ran over the **resolved** lines, i.e. after FIFO allocation had already split one
   request line across several batches. On a non-batch-tracked item — which gets no picker at all — it
   refused a quantity the user had no way to enter differently.

If shade matching is wanted back, note what (2) teaches: the rule belongs where the user's own choice
lives — on the **item** or the **job order step**, checked against what was picked — never on the
operation master and never downstream of the allocator.
...
5. **Several batches of one item are allowed.** A single-batch guard driven by
   `Process.requiresSingleBatch` blocked the second one until 2026-08-17; §5.2.4 records why the rule
   was real and the column was the wrong home for it. Nothing constrains the count now — a challan may
   draw one item from as many batches as the user picks, each with its own quantity and its own line.
...
processor and is written off as job order loss when the step is completed (§9.2). Appendix A works
...
| **Scrap**                    | Scrap is not typed on a receipt. Whatever the receipts did not use stays **still out at the processor** — for the next delivery — until a human completes the step or closes the order short. Then all of it is scrapped where it stands, same batch and package, at its running cost, as **job order loss** (Ind AS 2 ¶16(a)). It is never loaded back onto output batches that may already have moved on, and a completed step accepts no further document                                                                                                                                                                                                                                                                                                   |
...
that no inspection catches until assembly.

⚠️ **Nothing in the system stops this today.** `Process.requiresSingleBatch` was the guard and it was
dropped on 2026-08-17 (§5.2.4): it could not be set from any form, reset itself on every process edit,
and fired on FIFO-split lines the user never chose. The issue screen now lets both batches onto step 3's
challan. Whoever picks up shade matching next owns this example — and §5.2.4 says where the check belongs.
